...
prettyPosition pos =
  ...
  colName "pl"                      $$ nest nestCols (prettyAccountUnits $ Data.Oanda.Position.pl pos) $+$                      -- AccountUnits
  colName "unrealizedPL"            $$ nest nestCols (prettyAccountUnits $ Data.Oanda.Position.unrealizedPL pos) $+$            -- AccountUnits
  colName "marginUsed"              $$ nest nestCols (prettyAccountUnits $ marginUsed pos) $+$                                  -- AccountUnits
  colName "resettablePL"            $$ nest nestCols (prettyAccountUnits $ Data.Oanda.Position.resettablePL pos) $+$            -- AccountUnits
  colName "financing"               $$ nest nestCols (prettyAccountUnits $ Data.Oanda.Position.financing pos) $+$               -- AccountUnits
  colName "commission"              $$ nest nestCols (prettyAccountUnits $ commission pos) $+$                                  -- AccountUnits
  colName "guaranteedExecutionFees" $$ nest nestCols (prettyAccountUnits $ Data.Oanda.Position.guaranteedExecutionFees pos) $+$ -- AccountUnits
